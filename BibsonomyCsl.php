...
$custom_meta_fields = array(
    ...
    "searchtype" => array(
        'label' => 'Select query search type',
        'desc' => '<b>It is recommended to just leave it as \'Search Index\'</b>, as it supports more features. If there are any problems with the list of publications, you may try to change it to \'Local\'.',
        'id' => BibsonomyCsl::PREFIX . 'searchtype',
        'type' => 'select',
        'options' => array(
            'one' => array(
                'label' => 'Search Index',
...
class BibsonomyCsl
{
    ...
    public function bibsonomycsl_custom_fields_box_content($post_id)
    {
        ...
        $cslStyles = [
            ...
            "http://www.zotero.org/styles/everyAware.csl" => "BibSonomy – The BibSonomy default CSL style",
            ...
            "http://www.zotero.org/styles/lncs" => "Springer - Lecture Notes in Computer Science",
        ...
        foreach ($cslStyles as $key => $title) {
            $custom_meta_fields["stylesheet"]["options"][$key]["label"] = $title;
            $custom_meta_fields["stylesheet"]["options"][$key]["value"] = $key;
        }
    ...
    public function bibsonomycsl_insert_publications_from_post_meta($content)
    {
        ...
        $args['searchtype'] = get_post_meta($post->ID, BibsonomyCsl::PREFIX . 'searchtype', true);
